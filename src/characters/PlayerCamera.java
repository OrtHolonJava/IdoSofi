...
public class PlayerCamera 
{
	...
	public void setPosition()
	{
		 ...
		 */
		if (this._char.getObjBox().x + (this._panel.getWidth() / (2 * this._scale)) <= Map._sizeW * Map._blockSize
		...
		else
		{
			...
			{
				this._x += ((Map._sizeW * Map._blockSize - (this._panel.getWidth() / this._scale)) * -1 - this._x) / this._alpha;
			}
			else
			{
		...
		if (this._char.getObjBox().y + (this._panel.getHeight() / (2 * this._scale)) <= Map._sizeH * Map._blockSize)
		{
			this._y += ((this._char.getObjBox().y - (this._panel.getHeight() / (2 * this._scale))) * -1 - this._y) / this._alpha;
		}
		else
		{
			this._y += ((Map._sizeH * Map._blockSize - (this._panel.getHeight() / this._scale)) * -1 - this._y) / this._alpha;
		}
	}
